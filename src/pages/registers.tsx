import { apiAccount } from "@/api-client/account-api";
...
export default function RegisterPage(props: RegisterPageProps) {
  ...
  const handleRegister = async (values: any) => {
    console.log(values);
    try {
      const data = await apiAccount.register(values);
      console.log(data);
    } catch (error) {
      console.log(error);
    }
              ...
              >
                ...
                <Form.Item>
                  <div className="mt-10">
                    <button
                      type="submit"
                      className="outline-none border-none bg-primaryColor hover:scale-105 duration-300 ease-in-out rounded-md px-4 py-2 text-center text-white cursor-pointer"
                    >
                      Đăng nhập
                    </button>
                  </div>
